...
class ApiController extends Controller
{
    ...
    public function postMsg($data, $url)
    {
        try {
            ...
            return json_encode(['status' => false, 'msg' => '¡Asegúrese de que su servidor Node ya se esté ejecutando!']);
        }
    }
    ...
    public function generateQr(Request $request){
       ...
       // check user by api key
        ...
        if($user->is_expired_subscription){
            return response()->json([
                'status' => false ,
                'msg' => '¡Su suscripción ha expirado!',
            ],Response::HTTP_BAD_REQUEST);
        }
        if(!$user){
            return response()->json(['status' => false, 'msg' => '¡Clave de API incorrecta!'],Response::HTTP_BAD_REQUEST);
        }
        $number = Number::whereBody($request->number)->first();
        $allnumber = Number::whereUserId($user->id)->get();
        if(!$number){
            if($user->limit_device <= count($allnumber) ){
                return response()->json(['status' => false, 'msg' => '¡Has alcanzado tu límite de dispositivos!'],Response::HTTP_BAD_REQUEST);
            }
            $number = new Number();
        ...
        try {
            ...
            return response()->json(['status' => false, 'msg' => '¡Asegúrese de que su servidor Node ya se esté ejecutando!'],Response::HTTP_BAD_REQUEST);
        }
